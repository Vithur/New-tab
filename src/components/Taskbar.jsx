      ...
      {validShortcuts.map((s) => {
        ...
        return (
          <a
            ...
            className={`figma-glass-card h-[6.5vh] w-[6.5vh] min-h-[42px] min-w-[42px] rounded-full flex items-center justify-center text-white cursor-pointer transition-transform duration-300 ease-out ${
              isDragging
                ? 'opacity-40 scale-95'
                : isDragOver
                  ? 'scale-110'
            ...
            title={s.title || s.url}
            style={isDragOver ? { boxShadow: '0 0 20px rgba(255,255,255,0.35), inset 0 0 0 2px rgba(255,255,255,0.7)' } : undefined}
          >
            {s.iconDataUrl ||
            s.iconUrl ||
            (s.iconClass &&
